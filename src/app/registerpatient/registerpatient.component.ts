...
export class RegisterpatientComponent {
  ...
  onUpload(data:any) {
    ...
    formData.append('paper', this.selectedFile2,this.selectedFile2.name);
  ...
  onSelectFilepaper(fileInputt: any) {
    this.selectedFile2 = <File>fileInputt.target.files[0];
    console.log(this.selectedFile2)
    const dataTransferr = new ClipboardEvent('').clipboardData || new DataTransfer();
    dataTransferr.items.add(new File(['my-file'], 'new-file-name'));
    const inputElementt: HTMLInputElement = document.getElementById('formFilet') as HTMLInputElement

     inputElementt.files = dataTransferr.files;
  }
}
